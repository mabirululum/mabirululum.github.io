...
if ($method === 'GET') {
  ...
  $stmt = $pdo->prepare('SELECT * FROM hari_kegiatan WHERE tanggal BETWEEN ? AND ? ORDER BY tanggal ASC');
...
if ($method === 'POST') {
  ...
  $stmt = $pdo->prepare('INSERT INTO hari_kegiatan (tanggal, keterangan) VALUES (?, ?) ON DUPLICATE KEY UPDATE keterangan = VALUES(keterangan)');
...
if ($method === 'DELETE') {
  ...
  $pdo->prepare('DELETE FROM hari_kegiatan WHERE id = ?')->execute([$id]);
